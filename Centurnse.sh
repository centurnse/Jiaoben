...
show_progress() {
    ...
    echo -e "\n${BOLD}${WHITE}▛${BLUE} 阶段 ${current_step}/8 ${WHITE}» ${CYAN}${desc}${RESET}"
    echo -e "${BLUE}▌${RESET}"
}
...
update_system() {
    ...
    case $DISTRO in
        ubuntu|debian) $PM update -yqq > /dev/null ;;
        centos|rhel|fedora) $PM makecache --quiet > /dev/null ;;
        arch) pacman -Sy --noconfirm --quiet > /dev/null ;;
        opensuse) zypper --non-interactive refresh > /dev/null ;;
    esac

    echo -e "${BLUE}[2/3]${WHITE} 升级系统组件...${RESET}"
    case $DISTRO in
        ubuntu|debian) $PM upgrade -yqq > /dev/null ;;
        centos|rhel|fedora) $PM update -y --quiet > /dev/null ;;
        arch) pacman -Su --noconfirm --quiet > /dev/null ;;
        opensuse) zypper --non-interactive update -y > /dev/null ;;
    esac

    echo -e "${BLUE}[3/3]${WHITE} 清理过期软件包...${RESET}"
    case $DISTRO in
        ubuntu|debian) $PM autoremove -yqq > /dev/null ;;
        centos|rhel|fedora) $PM autoremove -y --quiet > /dev/null ;;
    esac
}
...
install_components() {
    ...
    case $DISTRO in
        ...
        arch)
            pacman -S --noconfirm --quiet "${components[@]}" > /dev/null ;;
        opensuse)
...
#!/bin/bash
find /var/log -type f $ -name "*.log" -o -name "*.gz" $ -exec truncate -s 0 {} \;
