...
case ${choice^^} in
    ...
    B)
        ...
        pretty_echo "检测到内存: ${total_mem}MB, 可用空间: ${free_space}MB"

        # 严格按照规则配置SWAP
        if [ $total_mem -lt 512 ] && [ $free_space -gt 5120 ]; then
            swap_size=512
            pretty_echo "RAM < 512MB 且 空间 > 5GB，设置512MB SWAP..."
        elif [ $total_mem -ge 512 ] && [ $total_mem -lt 1024 ] && [ $free_space -gt 8192 ]; then
            swap_size=1024
            pretty_echo "RAM 512MB-1GB 且 空间 > 8GB，设置1GB SWAP..."
        elif [ $total_mem -ge 1024 ] && [ $total_mem -lt 2048 ] && [ $free_space -gt 10240 ]; then
            swap_size=1024
            pretty_echo "RAM 1GB-2GB 且 空间 > 10GB，设置1GB SWAP..."
        elif [ $total_mem -ge 2048 ]; then
            pretty_echo "${GREEN}RAM ≥ 2GB，按照规则不设置SWAP${NC}"
            countdown
            # 直接跳到下一步
...
EOF
            ...
            pretty_echo "${YELLOW}不满足SWAP配置条件，跳过SWAP设置${NC}"
...
EOF
        ...
        # 只有满足条件才会执行以下SWAP创建代码
        pretty_echo "正在创建 ${swap_size}MB 的SWAP文件..."
        ...
        pretty_echo "${GREEN}✓ ${swap_size}MB SWAP配置完成${NC}"
        countdown
